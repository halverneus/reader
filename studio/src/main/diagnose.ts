...
import { probeEngine } from "./capture";
...
export async function diagnose(): Promise<Check[]> {
  ...
  if (cfg.capture.backend === "obs") {
    ...
    const p = await probeEngine();
    add("gst", "GStreamer capture engine", !p.error && !p.missing?.length, p.error ?? `${p.gstreamer} · python ${p.python}${p.missing?.length ? ` · missing ${p.missing.join(", ")}` : ""}`, "Needs the system python3-gobject and gstreamer1-plugins-good / -bad-free");
    add("encoder", "H.264 encoder", !!p.encoder && p.encoderWorks !== false, p.encoder ? `${p.encoder}${p.encoderWorks === false ? " (failed to open the GPU)" : ""}` : p.encoderError ?? "none", "nvh264enc needs the NVIDIA driver; vah264enc needs VA-API");
    add("portal", "Screen-cast portal", (p.portalScreenCast ?? 0) >= 4, p.portalScreenCast ? `ScreenCast v${p.portalScreenCast} · ${cfg.capture.restoreToken ? "VM window remembered" : "VM window not picked yet"}` : p.portalError ?? "unavailable", "xdg-desktop-portal-kde must be running");
    add("cam", "Camera device", fs.existsSync(cfg.capture.camDevice), cfg.capture.camDevice, "Plug in the phone and start its camera app");
    const mic = cfg.capture.micDevice ? { ok: (await sh("pactl", ["list", "sources", "short"])).out.includes(cfg.capture.micDevice), out: cfg.capture.micDevice } : await sh("pactl", ["get-default-source"]);
    add("mic", "Microphone", mic.ok, mic.out, "Plug in the mic, or pick one in Settings → Capture");
  }
