...
1. A "line" entry may carry \`mood: "<name>"\` (Glitch's face when the line starts). Inline tags inside any line's text switch the face mid-sentence: "Okay so… [surprise] Oh! OH. [excited] That's actually…". Tags are stripped before TTS. \`[pause 500]\` (milliseconds; bare \`[pause]\` = 500) puts a real silence in the spoken line and Glitch's mouth stops for it — Kokoro ignores "..." and commas, so write a pause tag wherever a beat matters: "You search the... [pause 400] I don't know... [pause 600] [dismay] There is no search." Tags in Dev's lines drive Glitch's *reaction* while Dev talks (Glitch listening, doubting, scheming).
2. \`- type: "face"\` with \`mood:\`, optional \`delay:\` (ms to wait before the face changes — use it when the reaction should land after Dev finishes reading the line) and optional \`hold:\` (ms, then back to the previous mood) for a reaction beat without dialogue.
...
4. Keystroke steps gained mouse actions: "m:move 960,540" (guest pixels; glides from the last position so viewers can follow it) or "m:move 50%,50%", "m:jump 960,540" (instant), "m:click", "m:click right", "m:dblclick", "m:down", "m:up", "m:drag x,y", "m:scroll -3".
...
const tools: Anthropic.Tool[] = [
  ...
  { name: "set_entry", description: "Create or replace the entry of a kind at a marker. kinds: line, editor, keys, face, slide, outro. For 'line' pass actor, text, optional mood. For 'outro' pass next (topic), thanks (array of names), optional phases (array of {phase, text, mood, title, subtitle}). For 'keys' pass keystrokes (array) and optional speed. For 'face' pass mood, optional delay (ms before the face changes) and optional hold (ms before it reverts). For 'slide' pass actor, to, optional over. Set auto=true to auto-advance when everything at the marker finishes. A marker may hold at most one entry per kind, except two 'line' entries only if actors differ (avoid).", input_schema: { type: "object", properties: { marker: { type: "integer" }, kind: { type: "string", enum: ["line", "editor", "keys", "face", "slide", "outro"] }, end: { type: "integer" }, auto: { type: "boolean" }, actor: { type: "string" }, text: { type: "string" }, mood: { type: "string" }, hold: { type: "integer" }, keystrokes: { type: "array", items: { type: "string" } }, speed: { type: "integer" }, delay: { type: "integer" }, to: { type: "string" }, over: { type: "integer" }, next: { type: "string" }, thanks: { type: "array", items: { type: "string" } }, phases: { type: "array", items: { type: "object", properties: { phase: { type: "string", enum: ["thanks", "credits", "next", "subscribe"] }, text: { type: "string" }, mood: { type: "string" }, title: { type: "string" }, subtitle: { type: "string" } }, required: ["phase"], additionalProperties: false } } }, required: ["marker", "kind"], additionalProperties: false } },
...
function runTool(name: string, input: any): string {
  ...
  switch (name) {
    ...
    case "set_entry": {
      ...
      else if (kind === "face") e = { ...base, type: "face", mood: input.mood ?? "neutral", hold: input.hold, delay: input.delay };
